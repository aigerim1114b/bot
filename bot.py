...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    poem = (
        ...
        "_Хоть \"кость\" зовут, нo не скелета часть,_\n"
...
async def handle_question_5(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if user_message and user_message.lower() ==  "дазай осаму":
        question_6_text1 = (
            "Не думал, что Вы доберетесь до Японии сами, но Вы смогли\\. Какой сюрприз\\. Мы находимся в баре *\"Люпин\"*, в который почему\\-то часто захаживает Дазай\\. Он то и дал Айко это задание,  вместе с Анго, сказав, что что\\-то среди этого может помочь найти того, кто всю эту заварушку и начал\\. Айко почему\\-то решила оставить документы задания здесь, видимо она их здесь и получила\\. Однако, чтобы получить эти документы, а они важны для задания, вам стоит открыть сейф при помощи кода, увы\\. Он довольно легкий\\. Айко оставила подсказку как найти код, смотрите:\n\n"
            "*1?2 12? 35? 249 5?63*\n\n"
        )
        question_6_text2 = (
